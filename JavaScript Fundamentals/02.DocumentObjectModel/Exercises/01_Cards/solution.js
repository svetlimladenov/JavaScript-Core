function solve() {
    ...
    function Fight(card1, card2) {
        ...
        let winnerSelector = document.getElementsByName(biggestValue.toString());
        winnerSelector.style.border = "2px solid green";
        let smallestValue = Math.min(card1Value,card2Value);
        let looserSelector = document.getElementsByName(smallestValue.toString());
        looserSelector.style.border = "2px solid darkred";
        ...
        history.textContent += "[" + spans[0].textContent + " vs " + spans[2].text + "] ";
    }
}